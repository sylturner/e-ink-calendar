...
class MonochromeCanvas
  def self.from_bmp(bmp)
    width = bmp.byteslice(18, 4).unpack1("V")
    height = bmp.byteslice(22, 4).unpack1("V")
    canvas = new(width, height)
    pixel_data = bmp.byteslice(62, canvas.instance_variable_get(:@row_bytes) * height)
    height.times do |row|
      source_offset = (height - 1 - row) * canvas.instance_variable_get(:@row_bytes)
      canvas.instance_variable_get(:@pixels)[row * canvas.instance_variable_get(:@row_bytes), canvas.instance_variable_get(:@row_bytes)] = pixel_data.byteslice(source_offset, canvas.instance_variable_get(:@row_bytes))
    end
    canvas
  end
  ...
  def draw_text(x, baseline, text, font:, black: true, anchor: nil, clip: nil)
    x = x.round
    ...
    text.each_codepoint do |codepoint|
      ...
      draw_glyph(x + glyph.x_offset, baseline.round - glyph.height - glyph.y_offset, glyph, black:, clip:)
      x += glyph.advance
    end
  end
  ...
  def draw_glyph(x, y, glyph, black:, clip:)
    row_bits = ((glyph.width + 7) / 8) * 8
    glyph.rows.each_with_index do |bits, row|
      glyph.width.times do |column|
        set_pixel(x + column, y + row, black:, clip:) if (bits & (1 << (row_bits - column - 1))).positive?
      end
    end
  end

  def set_pixel(x, y, black:, clip: nil)
    return unless x.between?(0, @width - 1) && y.between?(0, @height - 1)
    return if clip && !(x >= clip.fetch(:x) && x < clip.fetch(:x) + clip.fetch(:width) && y >= clip.fetch(:y) && y < clip.fetch(:y) + clip.fetch(:height))
